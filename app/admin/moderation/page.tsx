...
async function getData() {
  ...
  const [commentsRes, photosRes, profilesRes, placesRes] = await Promise.all([
    supabase.from('comments').select('id, content, created_at, user_id, place_id, is_hidden, deleted_at').order('created_at', { ascending: false }).limit(20),
    supabase.from('photos').select('id, image_url, created_at, user_id, place_id, is_hidden, deleted_at').order('created_at', { ascending: false }).limit(20),
            ...
            {comments.map((comment) => (
              <div key={comment.id} className='rounded-2xl border border-zinc-800 bg-zinc-950 p-4'>
                ...
                <div className='mt-4 flex flex-wrap gap-2'>
                  <button onClick={() => moderate('comments', comment.id, 'hide')} className='rounded-xl border border-zinc-700 px-3 py-2 text-sm text-white'>Hide</button>
                  <button onClick={() => moderate('comments', comment.id, 'soft_delete')} className='rounded-xl border border-yellow-800 bg-yellow-950 px-3 py-2 text-sm text-yellow-200'>Soft delete</button>
                  <button onClick={() => moderate('comments', comment.id, 'restore')} className='rounded-xl border border-emerald-800 bg-emerald-950 px-3 py-2 text-sm text-emerald-200'>Restore</button>
                  <button onClick={() => hardDelete('comments', comment.id)} className='rounded-xl border border-red-900 bg-red-950 px-3 py-2 text-sm text-red-200'>Hard delete</button>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className='mt-5 space-y-4'>
            {photos.map((photo) => (
              <div key={photo.id} className='rounded-2xl border border-zinc-800 bg-zinc-950 p-4'>
                ...
                <div className='mt-4 flex flex-wrap gap-2'>
                  <button onClick={() => moderate('photos', photo.id, 'hide')} className='rounded-xl border border-zinc-700 px-3 py-2 text-sm text-white'>Hide</button>
                  <button onClick={() => moderate('photos', photo.id, 'soft_delete')} className='rounded-xl border border-yellow-800 bg-yellow-950 px-3 py-2 text-sm text-yellow-200'>Soft delete</button>
                  <button onClick={() => moderate('photos', photo.id, 'restore')} className='rounded-xl border border-emerald-800 bg-emerald-950 px-3 py-2 text-sm text-emerald-200'>Restore</button>
                  <button onClick={() => hardDelete('photos', photo.id)} className='rounded-xl border border-red-900 bg-red-950 px-3 py-2 text-sm text-red-200'>Hard delete</button>
                </div>
              </div>
            ))}
          </div>
